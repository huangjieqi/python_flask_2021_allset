...
app.secret_key = '520'
import requests
def get_constellation(consName):
    key = "39afabac247a25466f47364bbe3ec9b1"
    payload = {
    'consName': consName,
    'type': 'today',
    'key':key
    ...
    req = requests.get('http://web.juhe.cn:8080/constellation/getAll', params=payload)
    return req.json()
...
def do_search():
    ...
    if not parse:
        return None
    results = get_constellation(phrase)
    print(results)
    # content = []
    # titles = []
    # if results:
    ...
    # print(content)
    name_1 = str(results['name']),
    datatime_1 = str(results['datetime']),
    all_1 = str(results['all']),
    QFriend_1 = str(results['QFriend']),
    color_1= str(results['color']),
    number_1 = str(results['number']),
    summary_1 = str(results['summary']),

    # titles = ('id地址','主要内容','详细信息')
    log_request(request, phrase)
    return render_template('view.html',
                           the_title=u'    哒哒哒~您的星座进入运势如下',
                           name = name_1,
                           datatime = datatime_1,
                           all = all_1,
                           QFriend = QFriend_1,
